...
from data_analysis.extraction import get_btc_last_3_swings_info, btc_last_swing_info
...
def auto_download_charts(
        ...
        num_of_charts: int = Query(..., title="Chart Download",
                                   description='''Please Enter the Number of Chart to download From 1 to 358 Note: Please set the date first before auto download''', example=298)
):
    try:
...
def analyse_excel_data(
    ...
    try:
        ...
        return {"Status": f"Complete Chart data Extraction Successful for {total_csv_files_in_folder} Excel Files in {total_time_min} Secs",
                "top_50_coins_lst": top_50_coins_lst,
